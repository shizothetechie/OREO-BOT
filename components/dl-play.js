...
const handler = async (m, { oreo, command, text, args, usedPrefix }) => {
  ...
  oreo.GURUPLAY = oreo.GURUPLAY ? oreo.GURUPLAY : {}
...
handler.before = async (m, { oreo }) => {
  oreo.GURUPLAY = oreo.GURUPLAY ? oreo.GURUPLAY : {}
  if (m.isBaileys || !(m.sender in oreo.GURUPLAY)) return
  const { result, key, timeout } = oreo.GURUPLAY[m.sender]
