...
let handler = async (m, { oreo, args, usedPrefix, command, isOwner, isPrems }) => {
  var limit
...
`.trim()
  await oreo.sendFile(m.chat, ss, 'ssweb.png', caption, m)

  if (!isLimit)
    await oreo.sendFile(m.chat, url, filename, '', m, null, { mimetype: ext, asDocument: true })
  m.react(done)
}
