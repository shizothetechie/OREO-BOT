...
let handler = async (m, { oreo, text }) => {
  try {
    ...
    try {
      await oreo.sendMessage(m.chat, await sticker.toMessage(), { quoted: m })
    } catch (stickerError) {
      console.error('Error sending sticker:', stickerError)
      m.reply('Error sending sticker. Sending image instead.')

      await oreo.sendFile(m.chat, tempImagePath, 'quote.png', 'Here is the quote image:', m)
    }
