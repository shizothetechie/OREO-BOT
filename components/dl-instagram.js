...
let handler = async (m, { oreo, usedPrefix, args, command, text }) => {
  ...
  for (const mediaData of mediaArray) {
    ...
    if (mediaType === 'video') {
      oreo.sendFile(m.chat, mediaURL, 'instagram.mp4', cap, m)
    } else if (mediaType === 'image') {
      oreo.sendFile(m.chat, mediaURL, 'instagram.jpg', cap, m)
    }
  }
}
