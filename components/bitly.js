...
let handler = async (m, { oreo, args }) => {
  try {
    ...
    if (!longURL) {
      return oreo.reply(m.chat, 'Please provide a long URL.', m);
    }
    const { shortURL, qrCode } = await bitly(longURL);
    await oreo.sendFile(m.chat, qrCode, 'qrcode.png', `Successfuly Shorted Your Link ✅
...
👨‍🎓 *Developer:* ${author}`, m);
    ...
    console.error(error);
    oreo.reply(m.chat, 'Failed to create short URL and QR code.\n' + error, m);
  }
};
