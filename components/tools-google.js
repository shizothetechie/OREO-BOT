...
const handler = async (m, { oreo, command, text, args, usedPrefix }) => {
  if (!text) throw `give a text to search Example: *${usedPrefix + command}* Guru bot`
  oreo.gogleit = oreo.gogleit ? oreo.gogleit : {}
  await oreo.reply(m.chat, wait, m)
  ...
  const { key } = await oreo.reply(m.chat, fullText, m)
  oreo.gogleit[m.sender] = {
    result,
    key,
    timeout: setTimeout(() => {
      oreo.sendMessage(m.chat, {
        delete: key,
      })
      delete oreo.gogleit[m.sender]
    }, 150 * 1000),
  }
}

handler.before = async (m, { oreo }) => {
  oreo.gogleit = oreo.gogleit ? oreo.gogleit : {}
  if (m.isBaileys || !(m.sender in oreo.gogleit)) return
  const { result, key, timeout } = oreo.gogleit[m.sender]
  console.log(oreo.gogleit)
  ...
  if (inputNumber >= 1 && inputNumber <= result.allLinks.length) {
    ...
    await oreo.sendFile(m.chat, response, 'google.jpg', 'Tada! Here is your result', m)
  } else {
    m.reply(
