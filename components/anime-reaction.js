...
let handler = async (m, { oreo, args, usedPrefix, command }) => {
  let who;
  if (m.isGroup) {
  ...
  let name2 = oreo.getName(m.sender);
  m.react(rwait);
...
oreo.sendMessage(m.chat, { video: { url: url }, gifPlayback: true, caption: `(${name2}) ${command} ${name}`, mentions: [m.sender] }, { quoted: m })
