...
let handler = async (m, { oreo, text }) => {
  ...
  let pp = await oreo.profilePictureUrl(data.id, 'image').catch(console.error)
  if (pp) {
    return oreo.sendMessage(m.chat, { image: { url: pp }, caption: txt }, { quoted: m })
  } else {
    let groupinfo = `❖ ID: ${data.id}\n❖ Name: ${data.subject}\n❖ Created on: ${data.creation}\n❖ Owner: ${data.owner}\n❖ Description:\n${data.desc}`
    await oreo.reply(m.chat, groupinfo, m)
  }
}
