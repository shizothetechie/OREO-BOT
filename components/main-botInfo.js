...
let handler = async (m, { oreo, usedPrefix, command }) => {
  const chats = Object.entries(oreo.chats).filter(([id, data]) => id && data.isChats)
  ...
  let pp = await oreo.profilePictureUrl(who, 'image').catch(_ => './media/shizo.jpg')
...
`
  oreo.sendFile(m.chat, pp, 'prefil.jpg', infobt, m, false, { mentions: [who] })
  m.react(done)
}
handler.help = ['info']
handler.tags = ['main']
