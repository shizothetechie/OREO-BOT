...
const handler = async (m, { oreo, args, usedPrefix, command }) => {
  /*if (global.db.data.users[m.sender].level < 5) {
        return oreo.reply(m.chat, 'You must be at least level 5 to use this command.', m);
    }*/

  const resolveRoulette = (chatId, oreo) => {
    let who = m.quoted
      ? m.quoted.sender
    ...
    if (rouletteBets[chatId] && rouletteBets[chatId].length > 0) {
      ...
      //oreo.sendFile(m.chat, pp, 'gamble.jpg', resultMessage, m, false, { mentions: [who] })
      oreo.reply(m.chat, resultMessage, m, { mentions: [who] })
      //m.reply(resultMessage)
    }
  }

  const runRoulette = (chatId, oreo) => {
    const delay = 10 * 1000 // 30 seconds

    setTimeout(() => {
      resolveRoulette(chatId, oreo)
    }, delay)
  }
