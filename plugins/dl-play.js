...
const handler = async (m, {
    ...
    if (!text) throw `*_give a text to search_* \n*_Example: ${usedPrefix + command} Khodi khodi dhodi ke kuwa kaila ba_*`;
    conn.OREOPLAY = conn.OREOPLAY ? conn.OREOPLAY : {};
    await conn.reply(m.chat, wait, m);
    const result = await searchAndDownloadMusic(text);
    const infoText = `*🖤⃝𓊈𒆜OREO PLAYER 𒆜𓊉࿐*
*╰─〔❨SELECT THE AUDIO ❩〕─╯*
*0:57 ————|——— -2:10*`;
...
handler.before = async (m, {
    ...
    conn.OREOPLAY = conn.OREOPLAY ? conn.OREOPLAY : {};
    if (m.isBaileys || !(m.sender in conn.OREOPLAY)) return;
    const {
        result,
        key,
        timeout
    } = conn.OREOPLAY[m.sender];
    console.log(conn.OREOPLAY)
    ...
    let title = generateRandomName();
        ...
        m.reply("Don't try to oversmart just select between 1 to " + result.allLinks.length);
    }
};
