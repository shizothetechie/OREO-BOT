...
let handler = async (m, { conn, command, args }) => {
    ...
    count = count ? /all/i.test(count) ? Math.floor(global.db.data.users[m.sender].bank / xpperrupees) : parseInt(count) : args[0] ? parseInt(args[0]) : 1
    count = Math.max(1, count)
    if (global.db.data.users[m.sender].bank >= xpperrupees * count) {
      global.db.data.users[m.sender].bank -= xpperrupees * count
