...
import { isUrl, resize, stream2buffer } from './corefunc.js'
...
/** 
 * @param {import('./connection').Socket} oreo 
 * @param {{
 *  store: typeof import('./connection')['default']['store']
 *  logger: import('./connection.js').Logger
 * }} options
 */
export function HelperConnection(oreo, { store, logger }) {
	const botUser = oreo.user || {}

	/** @type {import('@shizodevs/shizoweb').WASocket} */
	let sock = Object.defineProperties(oreo, {
		decodeJid: {
			value(jid) {
		...
		//                 else if (is()) {
		//                     oreo.ev.off(eventName, on)
		//                     resolve(...args)
		//                 }
		//             }
		//             oreo.ev.on(eventName, on)
		//         })
		//     }
		// },
		sendMsg: {
			async value(jid, message = {}, options = {}) {
				return await oreo.sendMessage(jid, message, { ...options, backgroundColor: '', ephemeralExpiration: 86400 })
			},
			enumerable: true,
			writable: true
		},
		sendPoll: {
			async value(jid, name = '', values = [], selectableCount = 1) {
				return await oreo.sendMessage(jid, { poll: { name, values, selectableCount }})
			},
			enumerable: true,
			writable: true
		},
		sendFThumb: {
			async value(jid, title, text = '', thumbnailUrl, sourceUrl, quoted, LargerThumbnail = true, AdAttribution = true) {
				return oreo.sendMsg(jid, { ...{
					contextInfo: {
						mentionedJid: await oreo.parseMention(text),
						externalAdReply: {
							title: title,
							body: null,
							mediaType: 1,
		...
		sendFile: {
			...
			async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}, isConvert = false) {
				const file = await oreo.getFile(path)
				let mtype = '',
					buffer = file.data,
				...
				let error = false
				try {
					return await oreo.sendMsg(jid, message, { ...opt, ...options })
				} catch (e) {
					console.error(e)
					error = e
				} finally {
		...
		sendContact: {
			 ...
			 * @param {import('@shizodevs/shizoweb').proto.WebMessageInfo} quoted 
			 * @param {Object} options 
			 */
			async value(jid, data, quoted, options = {}) {
				...
				for (let [number, name] of data) {
					...
					let biz = await oreo.getBusinessProfile(njid).catch(_ => null) || {}
					let vcard = `
BEGIN:VCARD
VERSION:3.0
N:;${name.replace(/\n/g, '\\n')};;;
...
X-WA-BIZ-NAME:${(store.getContact(njid)?.vname || oreo.getName(njid) || name).replace(/\n/, '\\n')}
...
`.trim()
		...
		reply: {
			...
			value(jid, text = '', quoted, options = {}) {
				return Buffer.isBuffer(text) ? oreo.sendFile(jid, text, 'file', '', quoted, false, options) : oreo.sendMsg(jid, { ...options, text }, { quoted, ...options })
			},
			writable: true,
		},
		preSudo: {
			async value(text, who, m, chatupdate) {
				let messages = await generateWAMessage(m.chat, { text, mentions: await oreo.parseMention(text) }, {
					userJid: who,
					quoted: m.quoted && m.quoted.fakeObj
				})
				messages.key.fromMe = areJidsSameUser(who, oreo.user.id)
				...
				let msg = {
					...chatupdate,
					messages: [proto.WebMessageInfo.fromObject(messages)].map(v => (v.oreo = this, v)),
					type: 'append'
				}
				return msg
			}
		...
		sendButton: {
			...
			async value(jid, text = '', footer = '', buffer, buttons, quoted, copy, urls, options = {}) {
				...
				if (buffer) {
					try {
						file = await oreo.getFile(buffer)
						// if not android, send normal message instead of button
						if (!isAndroid) return await oreo.sendFile(jid, buffer, '', text, quoted, false, options)
						else buffer = await prepareWAMessageMedia({ [/image/.test(file.mime) ? 'image'
						: 'video']: file.data }, { upload: oreo.waUploadToServer })
					} catch (e) {
						console.error(e)
						file = buffer = null
					}
				} else { if (!isAndroid) return await oreo.reply(jid, text, quoted) }
				...
				let msgL = generateWAMessageFromContent(jid, {
					viewOnceMessage: {
						message: {
							interactiveMessage } } }, { ephemeralExpiration: 86400, userJid: oreo.user.jid, quoted })
				return await oreo.relayMessage(jid, msgL.message, { messageId: msgL.key.id, ...options })
			},
			enumerable: true,
			writable: true,
		},
		sendSlide: {
			...
			async value(jid, text = '', footer = '', slides, quoted, options = {}) {
				...
				for (let [txt, ftr, header, thumbnailUrl, buttons] of slides.slice(0, 7)) {
					if (!isAndroid) return await oreo.sendButton(jid, txt, footer, thumbnailUrl, buttons, quoted)
					try {
						const mediaMessage = await prepareWAMessageMedia({
							[/\.mp4/i.test(thumbnailUrl) ? 'video' : 'image']: { url: thumbnailUrl }
						}, { upload: oreo.waUploadToServer });
						await array.push({
				...
				await oreo.relayMessage(jid, msg.message, { messageId: msg.key.id });
			},
			enumerable: true,
			writable: true,
		},
		sendHydrated: {
			...
			async value(jid, text = '', footer = '', buffer, url, urlText, call, callText, buttons, quoted, options = {}) {
				...
				let error = false
				try {
					return await oreo.sendMsg(jid, message, {
						quoted,
						upload: oreo.waUploadToServer,
						...options
					})
		...
		sendList: {
			async value(jid, title, text, footer, buttonText, buffer, listSections, quoted, btns, options = {}) {
				...
				}, { ephemeralExpiration: 86400, userJid: oreo.user.jid, quoted })
				return await oreo.relayMessage(jid, msgL.message, { messageId: msgL.key.id, ...options })
			}
		},
		cMod: {
			 ...
			 * cMod
			 * @param {String} jid 
			 * @param {import('@shizodevs/shizoweb').proto.WebMessageInfo} message 
			 * @param {String} text 
			 * @param {String} sender 
			 * @param {*} options 
			 * @returns 
			 */
			value(jid, message, text = '', sender = oreo.user.jid, options = {}) {
				...
				copy.key.fromMe = areJidsSameUser(sender, oreo.user.id) || false
		...
		copyNForward: {
			...
			async value(jid, message, forwardingScore = true, options = {}) {
				...
				m = generateWAMessageFromContent(jid, m, {
					...options,
					userJid: oreo.user.jid
				})
				await oreo.relayMessage(jid, m.message, { messageId: m.key.id, additionalAttributes: options })
				return m
			},
			enumerable: true,
			writable: true,
		...
		fakeReply: {
			...
			value(jid, text = '', fakeJid = this.user.jid, fakeText = '', fakeGroupJid, options = {}) {
				return oreo.reply(jid, text, { key: { fromMe: areJidsSameUser(fakeJid, oreo.user.id), participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options })
			},
			writable: true,
		},
		downloadM: {
			...
			async value(m, type, opts) {
				...
				if (opts.saveToFile) ({ fullpath } = await oreo.getFile(buffers, true))
		...
		getName: {
			...
			value(jid = '', withoutContact = false) {
				jid = oreo.decodeJid(jid)
				withoutContact = oreo.withoutContact || withoutContact
				let v
				if (jid?.endsWith('@g.us')) return (async () => {
					v = await store.fetchGroupMetadata(jid, oreo.groupMetadata) || {}
					...
					jid,
					vname: 'WhatsApp'
				} : areJidsSameUser(jid, oreo.user?.id || '') ?
					oreo.user :
		...
		loadMessage: {
			 ...
			 * @returns {import('@shizodevs/shizoweb').proto.WebMessageInfo}
			 */
			value(jid, id) {
		...
		sendGroupV4Invite: {
			...
			async value(jid, groupJid, inviteCode, inviteExpiration, groupName, jpegThumbnail, caption = 'Invitation to join my WhatsApp Group', options = {}) {
				...
				const media = await prepareWAMessageMedia({ image: (await oreo.getFile(jpegThumbnail)).data }, { upload: oreo.waUploadToServer })
				...
				const m = generateWAMessageFromContent(jid, message, { userJid: oreo.user?.id })
				await oreo.relayMessage(jid, m.message, { messageId: m.key.id, additionalAttributes: { ...options } })
				return m
			},
			enumerable: true,
		...
		serializeM: {
			 ...
			 * @param {import('@shizodevs/shizoweb').proto.WebMessageInfo} m
			 */
			value(m) {
				return smsg(oreo, m)
			},
		...
		user: {
			get() {
				Object.assign(botUser, oreo.authState.creds.me || {})
				return {
					...botUser,
...
/**
 * Serialize Message
 * @param {ReturnType<typeof makeWASocket>} oreo 
 * @param {import('@shizodevs/shizoweb').proto.WebMessageInfo} m 
 * @param {Boolean} hasParent 
 */
export function smsg(oreo, m, hasParent) {
	if (!m) return m
	/**
	 * @type {import('@shizodevs/shizoweb').proto.WebMessageInfo}
	 */
	let M = proto.WebMessageInfo
	m = M.fromObject(m)
	Object.defineProperty(m, 'oreo', { enumerable: false, writable: true, value: oreo })
	...
	if (m.message) {
		if (m.mtype == 'protocolMessage' && m.msg.key) {
			...
			protocolMessageKey.fromMe = areJidsSameUser(protocolMessageKey.participant, oreo.user.id)
			if (!protocolMessageKey.fromMe && areJidsSameUser(protocolMessageKey.remoteJid, oreo.user.id)) protocolMessageKey.remoteJid = m.sender
		}
	...
	try {
		if (protocolMessageKey && m.mtype == 'protocolMessage') oreo.ev.emit('messages.delete', { keys: [protocolMessageKey] })
	} catch (e) {
		console.error(e)
	}
	return m
...
export function serialize() {
	return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		isBaileys: {
			get() {
				return this.id?.length === 16 || this.id?.startsWith('3EB0') || this.id?.startsWith('SHIZOWEB') && this.id?.length === 18 || this.id?.startsWith('SHIZOWEB') && this.id?.length === 28 || false
			}
		},
		chat: {
			get() {
		...
		sender: {
			get() {
				return this.oreo?.decodeJid(this.key?.fromMe && this.oreo?.user.id || this.participant || this.key.participant || this.chat || '')
			},
			enumerable: true
		},
		fromMe: {
			get() {
				return this.key?.fromMe || areJidsSameUser(this.oreo?.user.id, this.sender) || false
			}
		},
		mtype: {
			get() {
		...
		quoted: {
			get() {
				...
				return this.id?.length === 16 || this.id?.startsWith('3EB0') || this.id?.startsWith('SHIZOWEB') && this.id?.length === 18 || this.id?.startsWith('SHIZOWEB') && this.id?.length === 28 || false
					...
					fromMe: {
						get() {
							return areJidsSameUser(this.sender, self.oreo?.user.jid)
						},
						enumerable: true,
					},
					text: {
					...
					name: {
						get() {
							...
							return sender ? self.oreo?.getName(sender) : null
						},
						enumerable: true
					...
					download: {
						value(saveToFile = false) {
							...
							return self.oreo?.downloadM(msg ? msg[mtype] : this.mediaMessage[mtype], mtype.replace(/message/i, ''), { saveToFile })
						},
						enumerable: true,
						configurable: true,
					},
					reply: {
						...
						value(text, chatId, options = {}) {
							return self.oreo?.reply(chatId ? chatId : this.chat, text, this.vM, options)
						},
						enumerable: true,
					},
					copy: {
						...
						value() {
							...
							return smsg(oreo, M.fromObject(M.toObject(this.vM)))
						},
						enumerable: true,
					},
					forward: {
						...
						value(jid, force = false, options = {}) {
							return self.oreo?.sendMessage(jid, {
					...
					copyNForward: {
						...
						value(jid, forceForward = false, options = {}) {
							return self.oreo?.copyNForward(jid, this.vM, forceForward, options)
						},
						enumerable: true,
					...
					cMod: {
						...
						value(jid, text = '', sender = this.sender, options = {}) {
							return self.oreo?.cMod(jid, this.vM, text, sender, options)
						},
						enumerable: true,
					...
					delete: {
						 ...
						 */
						value() {
							return self.oreo?.sendMessage(this.chat, { delete: this.vM.key })
						},
						enumerable: true,
					...
					react: {
						value(text) {
							return self.oreo?.sendMessage(this.chat, {
								react: {
									text,
									key: this.vM.key
								}
		...
		name: {
			get() {
				return !nullish(this.pushName) && this.pushName || this.oreo?.getName(this.sender)
			},
			enumerable: true
		},
		download: {
			value(saveToFile = false) {
				const msg = this.message || null
				const mtype = msg ? Object.keys(msg)[0] : this.mediaType
				return this.oreo?.downloadM(msg ? msg[mtype] : this.mediaMessage[mtype], mtype.replace(/message/i, ''), { saveToFile })
			},
			enumerable: true,
			configurable: true
		},
		reply: {
			value(text, chatId, options = {}) {
				return this.oreo?.reply(chatId ? chatId : this.chat, text, this, options)
			}
		},
		copy: {
			value() {
				const M = proto.WebMessageInfo
				return smsg(this.oreo, M.fromObject(M.toObject(this)))
			},
			enumerable: true
		},
		forward: {
			value(jid, force = false, options = {}) {
				return this.oreo?.sendMessage(jid, {
		...
		copyNForward: {
			value(jid, forceForward = false, options = {}) {
				return this.oreo?.copyNForward(jid, this, forceForward, options)
			},
			enumerable: true
		},
		cMod: {
			value(jid, text = '', sender = this.sender, options = {}) {
				return this.oreo?.cMod(jid, this, text, sender, options)
			},
			enumerable: true
		},
		getQuotedObj: {
			value() {
				...
				const q = proto.WebMessageInfo.fromObject(this.oreo?.loadMessage(this.quoted.sender, this.quoted.id) || this.oreo?.loadMessage(this.quoted.id) || this.quoted.vM)
				return smsg(this.oreo, q)
			},
			enumerable: true
		},
		getQuotedMessage: {
		...
		delete: {
			value() {
				return this.oreo?.sendMessage(this.chat, { delete: this.key })
			},
			enumerable: true
		},
		react: {
			value(text) {
				return this.oreo?.sendMessage(this.chat, {
					react: {
						text,
						key: this.key
					}
