...
END:VCARD
        ...
        relayWAMessage: {
            async value (shizomsgfull) {
                    if (shizomsgfull.message.audioMessage) {
                        await conn.sendPresenceUpdate('recording', shizomsgfull.key.remoteJid)
                    } else {
                        await conn.sendPresenceUpdate('composing', shizomsgfull.key.remoteJid)
                    }
                    var mekirim = await conn.relayMessage(shizomsgfull.key.remoteJid, shizomsgfull.message, { messageId: shizomsgfull.key.id })
                    conn.ev.emit('messages.upsert', { messages: [shizomsgfull], type: 'append' });
                    return mekirim
                }
        },
    /**
        ...
        pushMessage: {
            ...
            async value(m) {
                ...
                for (const message of m) {
                    try {
                        // if (!(message instanceof proto.WebMessageInfo)) continue // https://github.com/shizoweb/Baileys/pull/696/commits/6a2cb5a4139d8eb0a75c4c4ea7ed52adc0aec20f
                        if (!message) continue
