...
`.trim()
 ...
 /*
  Read More about this at
   ...
   https://dev.to/shizo_the_techie/whatsapp-quickreply-and-list-buttons-baileys-library-buttons-patch-and-message-filterization-455l
    ...
    */
    ...
    parseMention: {
      ...
      value(jid = "", withoutContact = false) {
        ...
        if (jid.endsWith("@g.us")) {
          ...
          if (!(v.name || v.subject)) {
            return conn.groupMetadata(jid).then(metadata => metadata || store.fetchGroupMetadata(jid, conn
              .groupMetadata)).then(metadata => metadata || {}).then(metadata => metadata.name || metadata
              .subject || PhoneNumber('+' + Number(parseInt(jid))).getNumber('international'));
          }
          return (v.name || v.subject || PhoneNumber('+' + Number(parseInt(jid))).getNumber('international'));
        }
        /*
        WhatsApp Channel support added by Shizo The Techie 
        lots of love 💗
        shizo loves you 💗👑
        Follow on GitHub: @shizothetechie or @shizo-devs
        */
        if (jid.endsWith("@newsletter")) {
          v = conn.chats[jid] || store.chats[jid] || {};
          if (!(v.name || v.subject)) {
            return conn.newsletterMetadata('jid', jid).then(metadata => metadata || {}).then(metadata => metadata
              .name || metadata.subject || PhoneNumber('+' + Number(parseInt(jid))).getNumber('international'));
          }
          return (v.name || v.subject || PhoneNumber('+' + Number(parseInt(jid))).getNumber('international'));
        }
        v = jid === "0@s.whatsapp.net" ? { jid, vname: "WhatsApp" } : areJidsSameUser(jid, conn.user.id) ? conn
          .user : conn.chats[jid] || store.chats[jid] || {};
        return (withoutContact ? "" : v.name ? v.name : v.subject ? v.subject : v.vname ? v.vname : v.notify ? v
          .notify : v.verifiedName ? v.verifiedName : PhoneNumber('+' + Number(parseInt(jid))).getNumber(
            'international'));
      },
      enumerable: true
    },
    loadMessage: {
       ...
       */
      value(messageID) {
        return (Object.entries(conn.chats).filter(([_, { messages }]) => typeof messages === 'object').find(([_,
          { messages }
        ]) => Object.entries(messages).find(([k, v]) => k === messageID || v?.key?.id ===
          messageID))?.[1]?.messages?.[messageID] || {});
      },
      enumerable: true
    },
